...
- 2026-06-18: Improved Figma-like image fill UX: selected image fills now render as a compact "图片" summary row in the Fill section, clicking it opens the existing fill popover with the image tab active, and choosing crop in the popover keeps the crop controls mounted immediately.
- 2026-06-18: Improved direct canvas crop UX: crop mode now reads computed background images, supports dragging `<img>` selections through `object-position`, and schedules a save after background-image wheel zoom.
- 2026-06-18: Fixed crop drag instability by previewing pan/resize against the canvas DOM during pointer movement, then committing to the GrapesJS model once on pointerup. Background-image crop mode now also shows a temporary translucent image overlay outside the selected viewport with four corner resize handles.
- 2026-06-18: Fixed crop overlay event ownership: the translucent crop overlay now receives pointer events, dragging from the image area outside the selected viewport is treated as crop pan/resize, and crop mouse/pointer events clear GrapesJS hover instead of letting underlying elements react.
- 2026-06-18: Removed the right fill popover's inline crop preview/zoom controls; crop adjustments are now owned by the canvas overlay while the popover stays focused on image upload/replacement and fill size selection.
- 2026-06-18: `StylePanel.tsx` is at 1647 lines after the image fill, number scrub, canvas/page state, selected-colors, color editor, color field, effect-control, stroke-control, layout-control, primitive, transform-control, and image-fill UX work; `image-fill-control.tsx` is down to 195 lines after removing the panel crop preview.
...
| 2026-06-18 | Phase 7 | Image fill still felt unlike Figma: image selections could display the color field, and crop controls were tied to inline panel rendering instead of the fill popover. | Added a compact image-fill summary row, routed clicks to the existing fill popover with the image tab active, synchronized controlled image-state updates when crop is selected, and mapped `<img>` fill size to `object-fit`. | `apps/web/src/components/grapesjs/StylePanel.tsx`, `apps/web/src/components/grapesjs/image-fill-control.tsx`, `apps/web/src/components/grapesjs/StylePanel.module.css`, `apps/web/tests/components/grapesjs/StylePanel.test.tsx`, `apps/web/tests/components/grapesjs/image-fill-control.test.tsx` |
| 2026-06-18 | Phase 7 | Direct canvas cropping only read inline `background-image` styles and did not support dragging `<img>` selections in their visible viewport. | Enhanced the GrapesJS crop handler to read computed background images, drag background fills via `background-position`, drag `<img>` fills via `object-position`, and schedule persistence after background wheel zoom. | `apps/web/src/components/grapesjs/GrapesjsEditor.tsx` |
| 2026-06-18 | Validation | The expanded HTML editor focused suite passes after the image-fill/crop UX work. `git diff --check` passes. Web typecheck remains blocked only by the known unrelated `ProjectView`, question-form export, and runtime export errors. | Ran focused Vitest for 19 HTML editor files (133 tests), `git diff --check`, and `pnpm --filter @open-design/web typecheck`. | `apps/web/tests/components/grapesjs/StylePanel.test.tsx`, `apps/web/tests/components/grapesjs/image-fill-control.test.tsx`, `apps/web/tests/components/grapesjs/color-editor-popover.test.tsx` |
| 2026-06-18 | Phase 7 | Crop drag could become unstable after the first movement because pointermove wrote GrapesJS model styles continuously, causing mid-drag rerenders and target hit-test drift. | Changed crop pan/resize to DOM preview during movement and single model commit on pointerup; hit testing now uses the selected element's rect, and background-image crop has a translucent overlay plus four corner resize handles. | `apps/web/src/components/grapesjs/GrapesjsEditor.tsx` |
| 2026-06-18 | Phase 7 | The right fill popover duplicated crop work with an inline image preview and zoom slider, while the desired Figma-like interaction belongs on the canvas. | Removed the panel crop preview/zoom controls and their dead CSS; the popover now keeps upload/replacement plus size mode selection, and crop mode is driven by the canvas overlay. | `apps/web/src/components/grapesjs/image-fill-control.tsx`, `apps/web/src/components/grapesjs/color-editor-popover.tsx`, `apps/web/src/components/grapesjs/StylePanel.module.css`, `apps/web/tests/components/grapesjs/image-fill-control.test.tsx` |
| 2026-06-18 | Validation | The expanded HTML editor focused suite passes after the crop overlay and panel simplification. `git diff --check` passes. Web typecheck remains blocked only by the known unrelated `ProjectView`, question-form export, and runtime export errors. | Ran focused Vitest for 19 HTML editor files (133 tests), `git diff --check`, and `pnpm --filter @open-design/web typecheck`. | `apps/web/tests/components/grapesjs/GrapesjsEditor.test.ts`, `apps/web/tests/components/grapesjs/StylePanel.test.tsx`, `apps/web/tests/components/grapesjs/image-fill-control.test.tsx` |
| 2026-06-18 | Phase 7 | Dragging the crop image outside the selected viewport could still fall through to GrapesJS element drag/hover because the crop overlay was visual-only. | Made the crop overlay a pointer-owning interaction layer, allowed overlay-origin pointerdown to start crop pan, and stopped crop pointer/mouseover events before GrapesJS hover/drag handlers run. | `apps/web/src/components/grapesjs/GrapesjsEditor.tsx` |
| 2026-06-18 | Validation | The expanded HTML editor focused suite passes after crop overlay event ownership. `git diff --check` passes. Web typecheck remains blocked only by the known unrelated `ProjectView`, question-form export, and runtime export errors. | Ran focused Vitest for 19 HTML editor files (133 tests), `git diff --check`, and `pnpm --filter @open-design/web typecheck`. | `apps/web/tests/components/grapesjs/GrapesjsEditor.test.ts`, `apps/web/tests/components/grapesjs/StylePanel.test.tsx`, `apps/web/tests/components/grapesjs/image-fill-control.test.tsx` |
