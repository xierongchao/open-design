...
const MAC_WINDOW_CHROME_CSS = `
  ...
  /* The expand button must sit fully clear of the macOS traffic-light drag
   * region: .macos-drag-bar covers the left 96px and intercepts pointer
   * events for window dragging, so a left:96px toggle (left edge flush with
   * the drag region) silently swallows clicks — the press becomes a drag.
   * Push it 14px past the drag region and lift its z-index above the drag
   * bar so it stays clickable. */
