...
export function registerProjectFileRoutes(app: Express, ctx: RegisterProjectFileRoutesDeps) {
  ...
  const { listFiles, listProjectFolders, createProjectFolder, copyProjectFile, deleteProjectFolder, renameProjectFolder, searchProjectFiles, readProjectFile, resolveProjectDir, resolveProjectFilePath, parseByteRange, renameProjectFile, deleteProjectFile, writeProjectFile, sanitizeName, ensureProject } = ctx.projectFiles;
  ...
  app.post('/api/projects/:id/files/copy', async (req, res) => {
    try {
      const { name } = req.body || {};
      ...
      /** @type {import('@open-design/contracts').CopyProjectFileResponse} */
      const body = result;
      res.json(body);
    } catch (err: any) {
      const message = String(err?.message || err);
