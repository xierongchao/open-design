...
- For cross-built Windows tester zips, use `tools-pack win build --portable --to zip` and distribute `portableZipPath`; do not manually zip cached `win-unpacked` directories.
- For Windows tester zips that show the splash screen and then exit on another machine, check archive path length before changing runtime code. The zip must not contain `resources/open-design-web-standalone/node_modules/.pnpm/next@...` entries; the afterPack report's `copiedWin32PathBudgetAudit.overBudgetCount` must be `0`.
